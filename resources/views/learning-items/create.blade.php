...
@section('title', 'Create Learning Item')
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white rounded-2xl shadow-xl p-8 border-l-4 border-blue-500">
        <form method="POST" action="{{ route('learning-items.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" id="title" required 
                    ...
                    value="{{ old('title') }}" placeholder="e.g., Learn React Hooks">
                @error('title')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" 
                    class="block w-full px-4 py-3 border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all @error('description') border-red-500 @enderror" 
                    placeholder="Describe what you want to learn...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-600 flex items-center">
                        <span class="mr-1">⚠️</span> {{ $message }}
                    </p>
            ...
            <div>
                ...
                <textarea name="links" id="links" rows="3" 
                    class="block w-full px-4 py-3 border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all @error('links') border-red-500 @enderror" 
                    placeholder="https://example.com/resource1&#10;https://example.com/resource2">{{ old('links') }}</textarea>
                @error('links')
                    <p class="mt-2 text-sm text-red-600 flex items-center">
                        <span class="mr-1">⚠️</span> {{ $message }}
                    </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="day" id="day" 
                        class="block w-full px-4 py-3 border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                        placeholder="Monday" value="{{ old('day') }}">
                </div>
